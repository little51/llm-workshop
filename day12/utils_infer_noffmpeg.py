...
def infer_process(
    ...
    if audio_data.ndim == 1:
        audio = torch.FloatTensor(audio_data).unsqueeze(0)  # [1, samples]
    else:
        # 如果是立体声，转换为单声道
        audio = torch.FloatTensor(audio_data.mean(axis=1)).unsqueeze(0)
...
def infer_batch_process(
    ...
    if sr != target_sample_rate:
        # 方法1：使用librosa重采样（推荐，质量好）
        try:
            import librosa
            # 将tensor转回numpy进行重采样
            audio_np = audio.cpu().numpy()[0]  # [samples]
            audio_resampled = librosa.resample(
                audio_np, 
                orig_sr=sr, 
                target_sr=target_sample_rate
            )
            audio = torch.FloatTensor(audio_resampled).unsqueeze(0).to(device)
            ...
            try:
                ...
                num_samples = int(len(audio_np) * resample_ratio)
                audio_resampled = signal.resample(audio_np, num_samples)
                audio = torch.FloatTensor(audio_resampled).unsqueeze(0).to(device)
                sr = target_sample_rate
            except ImportError:
                # 方法3：手动计算（最后手段）
                print("Warning: Using simple resampling, quality may be degraded")
                resample_ratio = target_sample_rate / sr
                num_samples = int(audio.shape[-1] * resample_ratio)
